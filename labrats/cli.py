...
_CFG_HELP = (
	"Config directory "
	"(default: $XDG_CONFIG_HOME/labrats)"
)
_SRC_HELP = "Data source: biorxiv, arxiv, all"
_API_HELP = (
	"Local API base URL, e.g. "
	"http://localhost:2276/v1"
)
_DB_HELP = (
	"SQLite DB path "
	"(default: $XDG_DATA_HOME/labrats/labrats.db)"
)
...
def _union_arxiv_cats(profiles: list[dict]) -> list[str]:
	return list({
		c
		for p in profiles
		for c in p.get("arxiv_categories", [])
	})
...
def run(
	...
	if needs_scrape:
		fetch_topics = {
			"arxiv_categories": _union_arxiv_cats(
				needs_scrape
			)
		}
		rprint(
			f"Fetching \\[{source}]: {start} to {end}"
		)
		all_papers = _fetch_from_sources(
		...
		for profile in needs_scrape:
			...
			rprint(
				f"{pname}: "
				f"saved {len(filtered)} papers"
			)
	else:
		rprint(
			"[dim]All profiles already scraped "
			"today — using DB[/dim]"
		)
	...
	for profile in profiles:
		...
		if to_eval:
			rprint(
				f"{pname}: evaluating "
				f"{len(to_eval)} papers"
			)
			new_cards = asyncio.run(
				run_pipeline(
					to_eval, personas, model, api_base
				)
			)
		...
		for paper in db_papers:
			res_map = load_results(
				conn, paper.doi, pname
			)
			results = [
				res_map[n]
...
def preview(
	...
	profiles = load_profiles(cfg)
	fetch_topics = {
		"arxiv_categories": _union_arxiv_cats(profiles)
	}
	today = date.today()
	start = str(today - timedelta(days=1))
	end = str(today)

	papers = _fetch_from_sources(
		start, end, source, fetch_topics
	)
...
def test(
	...
	cards = asyncio.run(
		run_pipeline([paper], personas, model, api_base)
	)
	cards = score_cards(cards)
	card = cards[0]
